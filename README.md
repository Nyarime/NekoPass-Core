...
NekoPass Lite 不只是加密流量——它让流量*看起来*就是 Cisco AnyConnect SSL VPN 会话。TLS 证书从真实 VPN 服务器镜像（类似 Reality），Portal 页面完整模拟 Cisco ASA。DPI 看到的是标准企业 VPN，仅此而已。
...
- **Reality-style TLS** — Certificate mirrored from real VPN server
